...
public class UserMongoDB {
    ...
    public boolean insertNewAdmin(TUser user) throws GeneralSecurityException, 
            UnsupportedEncodingException {

        if (this.isExistedUsername(user.username)) {
            return false;
        }

        String encryptPassword = encode_pw.encrypt(user.password);
        Date date = new Date();
        String strDate = DBUserContract.DATE_TIME_FORMATTER.format(date);
        Document document = new Document(FIELD_USERNAME, user.username)
                .append(FIELD_PASSWORD, encryptPassword)
                .append(FIELD_CREATE_DATE, strDate)
                .append(FIELD_UPDATE_DATE, strDate)
                .append(FIELD_TYPE, user.typeUser);
